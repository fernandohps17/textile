<div class="container">
    <div class="content">
        <div class="icon_principal">

            <div class="icon_principal_container">
                <div class="icon_principal_container_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M444-200h70v-50q50-9 86-39t36-89q0-42-24-77t-96-61q-60-20-83-35t-23-41q0-26 18.5-41t53.5-15q32 0 50 15.5t26 38.5l64-26q-11-35-40.5-61T516-710v-50h-70v50q-50 11-78 44t-28 74q0 47 27.5 76t86.5 50q63 23 87.5 41t24.5 47q0 33-23.5 48.5T486-314q-33 0-58.5-20.5T390-396l-66 26q14 48 43.5 77.5T444-252v52Zm36 120q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/></svg>
                </div>
                <section class="icon_principal_container_title">
                    <header>
                        <h2>Precios asequibles</h2>
                    </header>
                </section>
                <div class="icon_principal_container_subtitle">
                    <p>Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Ut elit tellus luctus.</p>
                </div>
                <div class="icon_principal_container_btn">
                    <a href="#">Aprender más 
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="m700-300-57-56 84-84H120v-80h607l-83-84 57-56 179 180-180 180Z"/></svg>
                    </a>
                </div>
            </div>





            <div class="icon_principal_container">
                <div class="icon_principal_container_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M720-120H280v-520l280-280 50 50q7 7 11.5 19t4.5 23v14l-44 174h258q32 0 56 24t24 56v80q0 7-2 15t-4 15L794-168q-9 20-30 34t-44 14Zm-360-80h360l120-280v-80H480l54-220-174 174v406Zm0-406v406-406Zm-80-34v80H160v360h120v80H80v-520h200Z"/></svg>
                </div>
                <section class="icon_principal_container_title">
                    <header>
                        <h2>Calidad superior</h2>
                    </header>
                </section>
                <div class="icon_principal_container_subtitle">
                    <p>Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Ut elit tellus luctus.</p>
                </div>
                <div class="icon_principal_container_btn">
                    <a href="#">Aprender más 
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="m700-300-57-56 84-84H120v-80h607l-83-84 57-56 179 180-180 180Z"/></svg>
                    </a>
                </div>
            </div>






            <div class="icon_principal_container">
                <div class="icon_principal_container_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M440-120v-80h320v-284q0-117-81.5-198.5T480-764q-117 0-198.5 81.5T200-484v244h-40q-33 0-56.5-23.5T80-320v-80q0-21 10.5-39.5T120-469l3-53q8-68 39.5-126t79-101q47.5-43 109-67T480-840q68 0 129 24t109 66.5Q766-707 797-649t40 126l3 52q19 9 29.5 27t10.5 38v92q0 20-10.5 38T840-249v49q0 33-23.5 56.5T760-120H440Zm-80-280q-17 0-28.5-11.5T320-440q0-17 11.5-28.5T360-480q17 0 28.5 11.5T400-440q0 17-11.5 28.5T360-400Zm240 0q-17 0-28.5-11.5T560-440q0-17 11.5-28.5T600-480q17 0 28.5 11.5T640-440q0 17-11.5 28.5T600-400Zm-359-62q-7-106 64-182t177-76q89 0 156.5 56.5T720-519q-91-1-167.5-49T435-698q-16 80-67.5 142.5T241-462Z"/></svg>
                </div>
                <section class="icon_principal_container_title">
                    <header>
                        <h2>Soporte 24/7</h2>
                    </header>
                </section>
                <div class="icon_principal_container_subtitle">
                    <p>Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Ut elit tellus luctus.</p>
                </div>
                <div class="icon_principal_container_btn">
                    <a href="#">Aprender más 
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="m700-300-57-56 84-84H120v-80h607l-83-84 57-56 179 180-180 180Z"/></svg>
                    </a>
                </div>
            </div>


        </div>
    </div>
</div>